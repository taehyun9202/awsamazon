...
const RightSidebarWrapper = ({ children, open, setOpen }) => {
  return (
    <>
      <div
        aria-live="assertive"
      ...
      >
        <div className="relative w-full h-full flex flex-col items-center space-y-4">
          ...
          >
            {/* modal content */}
            <div className="flex flex-col absolute right-0 top-0 max-w-lg w-full h-screen bg-white shadow-lg pointer-events-auto ring-1 ring-black ring-opacity-20">
              {children}
            </div>
          </Transition>
        </div>
